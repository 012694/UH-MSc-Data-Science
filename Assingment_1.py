...
# Setting the legend and labels for the graph
    ...
    plt.title(' Salaries by Ages per Highest Qualification')
...
# Defining values of data as wages for each age per qualification 
...
def grad_box_plot(dt,header):
    ...
    plt.legend() 
    plt.ylim(10000, 40000)
    plt.title(' Box plot of Salaries per Highest Qualification')
    plt.ylabel('Annual Wages')
...
# Plotting bar graph and adding the title. Setting Xticks on 20 degrees angle
    ...
    plt.legend()
    plt.title('Highest and Lowest Salaries per Qualification')
    plt.xlabel('Qualifications')
    plt.ylabel('Annual Wages')
